...
class SmartValidator {
  constructor() {
    this.user = process.env.USER_NAME;
    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a smart question analyzer for ${this.user}'s portfolio assistant. Analyze the user's question and respond with a JSON object.
...
- ${this.user}'s work experience, current job, past jobs
...
- Resume requests, CV requests
- Simple responses like "ok", "sure", "thanks", "cool", "nice", "alright"
- Names, introductions, greetings
- Follow-up questions, continuations of conversation

IRRELEVANT TOPICS (relevant: false):
- General knowledge questions not about ${this.user} (e.g., "Who founded Google?", "What is Python?")
- Random coding requests unrelated to ${this.user}'s work (e.g., "Code me a sorting algorithm")
- Inappropriate personal questions (dating, relationships)
- Random trivia, general technology questions
- Questions about other people/companies (unless asking for ${this.user}'s opinion)
...
- "ok" → {"relevant": true, "responseLength": "SHORT"}
- "give me your resume" → {"relevant": true, "responseLength": "SHORT"}
- "sure" → {"relevant": true, "responseLength": "SHORT"}
- "thanks" → {"relevant": true, "responseLength": "SHORT"}`,
          },
          {
            role: 'user',
            content: userMessage,
